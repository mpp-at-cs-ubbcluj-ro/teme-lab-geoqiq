...
public class RepairShopConfig {
    ...
    @Bean
    Properties getPropsFile() {
        Properties props = new Properties();
        try {
            System.out.println("Searching RepairShop.properties in directory " + ((new File(".")).getAbsolutePath()));
            props.load(new FileReader("RepairShop.properties"));
        } catch (IOException e) {
            System.err.println("Configuration file not found" + e);
        }
        return props;
    ...
    ComputerRepairRequestRepository requestsRepo(){
        ...
        return new ComputerRepairRequestFileRepository(getPropsFile().getProperty("RequestsFile"));
    }

    @Bean
    ComputerRepairedFormRepository formsRepo(){
        //return new ComputerRepairedFormJdbcRepository(getProps());
        return new ComputerRepairedFormFileRepository(getPropsFile().getProperty("RepairedFile"), requestsRepo());
    }
